...
function ArticleExam() {
  ...
  const handleStatusChange=async(row)=>{
    try {
      console.log("Inside", row.id)


      const response = await axios.put(`http://localhost:4000/api/updatearticlestatus/${row.id}`);
  ...
  const column = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Link to={`/addProductArticle/${row.id}`}  
          >
              <button 
