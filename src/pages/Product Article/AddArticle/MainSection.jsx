...
function MainSection({onLive, onDraft}) {
  ...
  const submitoverview=async(e)=>{
    e.preventDefault();
      ...
      try {
        ...
        const result = await axios.post( "http://localhost:4000/api/addArticle", formData)
...
useEffect(() => {
  let title = 'https://medicaljagat.com/';
    ...
    <div>
      <form className="space-y-4" onSubmit={submitoverview}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                             <div className="input-area" style={{ width: '30%' }}>
                                 <label htmlFor="select" className="form-label">Product Type<span style={{ color: 'red' }}>*</span></label>
                                 <Select options={options}  closeMenuOnSelect={true} placeholder="Select options..." value={options.find((selectedOption) => selectedOption.value === ptype)} onChange={(selectedOption)=>setPtype(selectedOption ? selectedOption.value : null)} />
                             </div>
                             {visible && (
                                 <>
                                     <div className="input-area" style={{ width: '30%' }}>
                                   <label htmlFor="select" className="form-label">Product Page All<span style={{ color: 'red' }}>*</span></label>
                                   <Select options={optionsall}   closeMenuOnSelect={true} placeholder="Select options..." value={options.find((selectedOption) => selectedOption.value === pall)} onChange={(selectedOption)=>setPall(selectedOption ? selectedOption.value : null)}  />
                              </div>
                                     <div className="input-area" style={{ width: '30%' }}>
                                         <label htmlFor="select" className="form-label">Product Location<span style={{ color: 'red' }}>*</span></label>
                                         <Select options={optionsstate}  closeMenuOnSelect={true} placeholder="Select options..." value={options.find((selectedOption) => selectedOption.value === ploc)} onChange={(selectedOption)=>setPloc(selectedOption ? selectedOption.value : null)} />
                                     </div>
                                 </>
                             )}
                             { !visible && (
                              <>
                              <div className="input-area" style={{ width: '30%' }}>
                                         <label htmlFor="select" className="form-label">Product Page<span style={{ color: 'red' }}>*</span></label>
                                         <Select options={optionss}  closeMenuOnSelect={true} placeholder="Select options..." value={options.find((selectedOption) => selectedOption.value === ppage)} onChange={(selectedOption)=>setPpage(selectedOption ? selectedOption.value : null)} />
                                     </div>
                              </>
                             )
