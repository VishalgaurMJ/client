...
function ColOverUpdate() {
  ...
  const submitoverview=async(e)=>{
    e.preventDefault();
      ...
      try {
        ...
        formData.append('colover', colname);
        ...
        const result = await axios.put( `http://localhost:4000/api/UpdateCollegeOverview/${colname}`, formData)
  ...
  const saveAsDraft=async(e)=>{
    ...
    try {
      ...
      const result = await axios.post( "http://localhost:4000/api/coloverDetails", formData)
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('College Details:', data);
          ...
          setCs(data.coloverDetails[0].colover)
          setShortDesc(data.coloverDetails[0].shortdesc)
          setLongDesc(data.coloverDetails[0].longdesc)
          setTime(data.coloverDetails[0].time)
          setAuthName(data.coloverDetails[0].authname)
          setBreadt(data.coloverDetails[0].breadt)
          setPaget(data.coloverDetails[0].paget)
          setMetat(data.coloverDetails[0].metat)
          setMetadesc(data.coloverDetails[0].metadesc)
          setMetakey(data.coloverDetails[0].metakey)
          setOgt(data.coloverDetails[0].ogt)
          setOgdesc(data.coloverDetails[0].ogdesc)
          setOgrobot(data.coloverDetails[0].ogrobot)
          setOggogle(data.coloverDetails[0].oggogle)
          setNotifications(data.coloverDetails[0].notifications)
      } else {
  ...
  return (
    ...
    <form className="space-y-4" onSubmit={submitoverview}>
                        ...
                        <div className="input-area">
                            <label htmlFor="select" className="form-label">College Overview<span style={{marginLeft:'50px'}}>{colname}</span></label>
