...
function Overview({ colname, onSubmitt }) {
  // const[colover, setColover]=useState('')
  ...
  const submitoverview = async (e) => {
    ...
    try {
      ...
      const result = await axios.post(
        "https://server-dashboard-zeta.vercel.app/api/coloverdetails",
        formData
      );
  ...
  return (
    <div>
      <form className="space-y-4" onSubmit={submitoverview}>
        <div className="">
          <div className="input-area">
            <label htmlFor="select" className="form-label">
              College Overview
              <span style={{ marginLeft: "50px" }}>{colname}</span>
            </label>
          ...
          <div className="flex items-center" style={{ marginTop: "10px" }}>
            ...
            <input
              type="text"
              className="form-control"
              style={{ width: "500px", marginLeft: "-1000px" }}
              value={time}
        ...
        <button
          className="btn inline-flex justify-center btn-outline-dark ml-3 bg-green-400"
          type="submit"
        >
          Make Exam Live
        </button>
