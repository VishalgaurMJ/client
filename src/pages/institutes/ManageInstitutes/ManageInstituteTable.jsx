...
function ManageInstituteTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log("Inside Manage Institute Page")
      try {
        const response = await axios.get('http://localhost:4000/api/getinsdetails');
        ...
        if (response.status === 200) {
          ...
          const combinedData = insDetails.map(detail => ({
            ...detail,
            ...insoverdetails.find(overview => overview.colover === detail.colfull)
          }));
  ...
  const handleStatusChange=async(row)=>{
    try {
      ...
      const response = await axios.put(`http://localhost:4000/api/updateinsstatus/${row.colfull}`);
...
export default ManageInstituteTable
