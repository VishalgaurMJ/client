...
function ColGenUpdate({onShift, setColname}) {
  ...
  const handleSubmit=async(e)=>{
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:4000/api/insgenupdatedetails/${colfull}`, formData)
...
useEffect(() => {
  const fetchCollegeDetails = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/getinscompletedinfo/${colfull}`);
      if (response.ok) {
