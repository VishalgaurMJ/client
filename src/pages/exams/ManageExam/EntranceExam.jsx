...
const EntranceExam = () => {
  ...
  const filterexam = (formattedData) => {
    const filteredExams = formattedData.filter((item) => item.examType === 1);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const combinedData = examDetails.map((detail) => ({
          ...detail,
          ...overviewDetails.find((overview) => overview.title === detail.name),
        }));
  ...
  const handleDelete = async (value) => {
    try {
      ...
      const response = await axios.delete(
        `https://server-medicaljagat-git-main-medical-jagat.vercel.app/deletedetails/${value.action}`
      );
      toastii();
      if (response.status === 200) {
