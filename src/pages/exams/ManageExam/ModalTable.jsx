    ...
    Cell: ({ row }) => {
      ...
      return (
        <div className="flex space-x-3 rtl:space-x-reverse">
          ...
          <Link to={`/addexam/${value.action.split(" ")[1]}`}>
            <Tooltip
              content="Edit"
              placement="top"
              arrow
      ...
      if (response.status === 200) {
        ...
        if (Array.isArray(childDetails)) {
          ...
          const itemsToShow = childDetails.filter(
            (item) => filterChild === item.Exam_name // Match against each item's name
          );

          // Map the filtered items to the desired format
          const formattedData = itemsToShow.map((item, index) => ({
            id: index + 1,
            ChildName: item.et, // Adjust if the structure is different
            action: `${item._id} ${item.Exam_name} ${item.sl}` || "N/A",
