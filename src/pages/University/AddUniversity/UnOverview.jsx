...
function UnOverview({unname, onSubmitt}) {
  ...
  const submitoverview=async(e)=>{
    e.preventDefault();
      ...
      try {
        ...
        const result = await axios.post( "http://localhost:4000/api/unioverdetails", formData)
    ...
    <div>
      <form className="space-y-4" onSubmit={submitoverview}>
                          ...
                          <div className="input-area">
                              <label htmlFor="select" className="form-label">University Overview<span style={{marginLeft:'50px'}}>{unname}</span></label>
                              {/* <input type="text" className="form-control" placeholder="" value={unover} onChange={(e)=>setunover(e.target.value)} required/> */}
