...
function ManageUniversity() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/getUnidetails');
        if (response.status === 200) {
          const { universitydetails, universityoverDetails } = response.data[0];
          // Merge colgendetails and coloverdetails based on a unique identifier, e.g., id
          const combinedData = universitydetails.map(detail => ({
            ...detail,
            ...universityoverDetails.find(overview => overview.unover === detail.unfull)
          }));
  ...
  const handleStatusChange=async(row)=>{
    try {
      ...
      const response = await axios.put(`http://localhost:4000/api//updatecolstatus/${row.unover}`);
    ...
    const column = [
        ...
        {
          name: 'Full Name',
          selector: (row)=>row.unfull,
          sortable: true,
          width: '400px' 
        },
        {
          name: 'Name',
          selector:(row)=>row.unshort,
          sortable: true,
          width: '150px' 
        },
        {
